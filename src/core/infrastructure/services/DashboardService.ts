...
class DashboardService {
  async getStats(): Promise<DashboardStats> {
    ...
    return {
      ...
      upcomingAppointments: this.getUpcoming(appointments, clients, services, tomorrow),
  ...
  private countAppointments(apts: Appointment[], from: Date, to: Date): number {
    return apts.filter(a => a.date >= from && a.date < to).length;
  }
  ...
  private countAppointmentsThisWeek(apts: Appointment[], now: Date): number {
    ...
    return apts.filter(a => a.date >= start).length;
  }

  private countAppointmentsThisMonth(apts: Appointment[], now: Date): number {
    const first = new Date(now.getFullYear(), now.getMonth(), 1);
    return apts.filter(a => a.date >= first).length;
  }
  ...
  // 32 slots/día (8 horas × 4 slots/hora)
  private occupancyRate(apts: Appointment[], today: Date, tomorrow: Date): number {
    ...
    return Math.min(100, Math.round((occupied / 32) * 100));
  }
  ...
  private confirmationRate(apts: Appointment[], now: Date): number {
    ...
    const confirmed = thisMonth.filter(a => a.status === AppointmentStatus.CONFIRMED).length;
  ...
  private getTopServices(
    ...
    const completed = apts.filter(
      a => a.date >= first && a.status === AppointmentStatus.COMPLETED
    );
  ...
  private byWeek(apts: Appointment[], now: Date): WeekData[] {
    return [3, 2, 1, 0].map(weeksAgo => {
      ...
      return {
        ...
        count: apts.filter(a => a.date >= start && a.date < end).length,
      };
    });
  }
